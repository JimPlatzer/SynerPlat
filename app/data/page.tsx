...
export default function DataPage(){
 ...
 useEffect(()=>{fetch('/research-library.json').then(r=>r.json()).then((data:unknown)=>{const payload=data as {items?:Resource[]};setResources(Array.isArray(payload.items)?payload.items:[])})},[]);
 ...
 return <main className="data-page"><header className="topbar"><Link className="brand designed" href="/"><span>Syner</span>Plat</Link><nav><Link href="/">情报首页</Link><a href="#overview">数据概览</a><a href="#timeline">重点时间轴</a></nav><Link className="outline data-back" href="/">← 返回首页</Link></header>
