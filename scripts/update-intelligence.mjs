...
function dateParts(value){if(!value)return '';const calendarDate=String(value).match(/^(\d{4}-\d{2}-\d{2})/);if(calendarDate)return calendarDate[1];const date=new Date(value);return Number.isNaN(date.valueOf())?'':isoDate(date)}
...
async function main(){
 ...
 const storyUrls=new Set(stories.map(item=>item.url));const storyIds=new Set(stories.map(item=>item.id));const previousWatchlist=(previous.watchlist||[]).filter(item=>inWindow(item)&&meaningfulEsgNexus(item));
 const newWatchlist=verified.filter(item=>inWindow(item)).map(storyFromCandidate);
 const watchlist=uniqueByUrl([...previousWatchlist,...newWatchlist]).map(item=>{const reviewed=reviewedLibraryById.get(item.id);return reviewed?{...item,event:'同行评审研究',summary:reviewed.summary||item.summary,score:Math.max(7,Number(item.score)||0),dateNote:reviewed.dateNote||item.dateNote,source:reviewed.publisher||item.source,url:reviewed.url||item.url,tags:Array.from(new Set([...(reviewed.tags||[]),'同行评审'])).slice(0,5)}:item}).filter(item=>!storyUrls.has(item.url)&&!storyIds.has(item.id)).sort((a,b)=>Number(b.score)-Number(a.score)||activityDate(b).localeCompare(activityDate(a))).slice(0,12);
 ...
 const keywordMap=new Map([...previousKeywordEvents,...periodKeywordEvents,...candidateKeywordEvents,...libraryKeywordEvents,...visibleKeywordEvents].filter(item=>item.id&&item.date>=rollingMonthStart&&item.date<=endDate).map(item=>[item.id,item]));
