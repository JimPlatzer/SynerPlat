...
- `public/period-highlights.json` 保存月度和年度重点，并为30天滚动词频提供已核验基线。
- 自动任务仅处理能够打开原始页面的候选；模型不可用或证据不足时会保留已有数据，不会把搜索摘要直接写入网站。

### 当前筛选逻辑

1. 抓取官方、监管和受信学术元数据；
2. 打开原始链接，校验发布日期与事件日期；
3. 对 ESG 相关性、影响范围、实际后果、来源权威性和新颖性评分；
4. 政策与监管达到 6/10、学术线索达到 8/10 时进入带 ★ 的重大动态；达到 5/10 的其余已核验内容进入紧凑“新增观察”；
5. 去重后写入资料库和滚动关键词事件，再自动构建与公开发布。
